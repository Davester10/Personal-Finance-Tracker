...
function setupMonthYearFilter() {
  ...
  monthEl.addEventListener('change', () => { filterMonth=parseInt(monthEl.value); window.renderExpenses(); });
  yearEl.addEventListener('change', () => { filterYear=parseInt(yearEl.value); window.renderExpenses(); });
}

function getFiltered() {
  return transactions.filter(tx => { const d=new Date(tx.date); return d.getMonth()===filterMonth && d.getFullYear()===filterYear && tx.type==='expense'; });
}
...
window.renderExpenses = function() {
  ...
  document.getElementById('expensePageTotal').textContent = formatCurrency(total, sym);
  document.getElementById('expensePageCount').textContent = getFiltered().length;

  const list = document.getElementById('expenseList');
  if(!items.length){ list.innerHTML = '<div class="text-center py-10 text-slate-400 text-sm">No expense transactions found.</div>'; return; }
...
function txItemHTML(tx, sym) {
  ...
  return `
    <div class="flex items-center gap-3.5 px-2 py-3 rounded-xl hover:bg-slate-50 transition-colors">
      <div class="w-9 h-9 rounded-full bg-expense-100 text-expense-600 flex items-center justify-center text-sm"><i class="fa-solid fa-arrow-down"></i></div>
      ...
      <div class="text-right shrink-0">
        ...
        <div class="text-sm font-bold text-expense-600">-${formatCurrency(tx.amount, sym)}</div>
      </div>
